...
class ZoneObject(object):
    ...
    def __init__(self, zone_name, lines=None, path=None, zone_obj=None):
    ...
    def get_serial(self, date_obj=None):
        if self.zone_obj:
            self.serial = self._serial_from_zonefile(self.zone_obj)
            return self.serial
        if self.file_exists is False:
            ...
            self.serial = self._serial_from_zonefile(zone)
        return self.serial
